...
class TelegramUploader:
    ...
    async def _msg_to_reply(self):
        if self._listener.up_dest:
            # up_dest is set (LEECH_DUMP_CHAT or user dump) — upload there as usual
            msg_link = (
...
┖ <b>Source :</b> <a href='{self._listener.source_url}'>Click Here</a>"""
            ...
            # ---------------------------------------------------------------
            # NO up_dest set + command came from a GROUP
            # Force upload to user's PM instead of replying in the group
            # ---------------------------------------------------------------
...
┖ <b>Source :</b> <a href='{self._listener.source_url}'>Click Here</a>"""
            try:
                # Send the "Leech Started" anchor message to user's PM
                self._log_msg = await TgClient.bot.send_message(
                ...
                self._sent_msg = self._log_msg
                self._is_private = True  # PM is always private
                if self._user_session:
                    self._sent_msg = await TgClient.user.get_messages(
                        chat_id=self._sent_msg.chat.id,
                        message_ids=self._sent_msg.id,
            ...
            # Command was in PM already — reply normally
    ...
    async def _send_screenshots(self, dirpath, outputs):
        ...
        for i in range(0, len(inputs), 10):
            batch = inputs[i : i + 10]
            if Config.BOT_PM and not self._is_private:
    ...
    async def _copy_media(self):
        try:
            if self._bot_pm and not self._is_private:
