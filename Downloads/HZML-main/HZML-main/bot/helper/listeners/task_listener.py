 ...
 """,
    ...
    async def on_upload_complete(
        ...
        if self.is_yt:
            ...
            # Determine where to send the leeched files:
            # Priority: LEECH_DUMP_CHAT (global dump) > user PM (fallback when in group)
            # If command came from a group and no dump is set, always send to user's PM.
            if Config.LEECH_DUMP_CHAT:
                # Global dump is configured — send files there (original behaviour)
                log_chat = self.message  # notify group, files go to dump via TelegramUploader
                send_to_pm = False
            elif self.is_super_chat:
                # No dump set and command came from a group — force send to user PM
                log_chat = self.user_id
                send_to_pm = True
            else:
                # Already in PM — send normally
                log_chat = self.message
                send_to_pm = False

            if send_to_pm:
                # Notify group that files were sent to PM
                pm_notify = msg
                pm_notify += "〶 <b><u>Action Performed :</u></b>\n"
                pm_notify += "⋗ <i>File(s) have been sent to your PM</i>\n\n"
                await send_message(self.message, pm_notify)
            elif self.bot_pm:
                pmsg = msg
