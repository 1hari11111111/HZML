...
class TaskConfig:
    ...
    async def before_start(self):
        ...
        if not self.is_leech:
            ...
            # ---------------------------------------------------------------
            # LEECH DESTINATION LOGIC
            # Priority:
            #   1. User-specified -up dest or user's LEECH_DUMP_CHAT → leech_dest
            #   2. Global Config.LEECH_DUMP_CHAT → up_dest
            #   3. No dump set + command from group → up_dest = user PM (user_id)
            #   4. No dump set + command from PM → up_dest = "" (reply in PM)
            # ---------------------------------------------------------------
            self.leech_dest = self.up_dest or self.user_dict.get("LEECH_DUMP_CHAT")
            if Config.LEECH_DUMP_CHAT:
                # Global dump configured — use it as the primary upload destination
                self.up_dest = Config.LEECH_DUMP_CHAT
            elif self.is_super_chat:
                # No global dump + command came from a group → send to user's PM
                self.up_dest = self.user_id
            else:
                # Command already came from PM → upload there (empty = use message chat)
                self.up_dest = ""
